...
public class Panel : IActionListener, IChatable
{
        ...
        if (cmtoX >= wScroll - 10 && cmx >= wScroll - 10 && position == 0)
        {
            ...
            if (isChangeZone)
            {
                ...
                if (Char.myCharz().cHP > 0 && Char.myCharz().statusMe != 14)
                {
                    ...
                    {
                        Xmappp.SelectMapTrans(selected);
    ...
    private void paintPetStatusInfo(mGraphics g)
    {
        ...
        mFont.tahoma_7_yellow.drawString(g, mResources.critical + ": " + NinjaUtil.getMoneys(Char.myPetz().cCriticalFull) + "   " + mResources.armor + ": " + NinjaUtil.getMoneys(Char.myPetz().cDefull), X + 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
        ...
        if (currentTabIndex < currentTabName.Length - ((GameCanvas.panel2 == null) ? 1 : 0) && type != 17)
        {
            ...
            if (currItem != null)
            {
                if (currItem.isBuySpec)
                {
                    ...
                    myVector.addElement(new Command("Mua nhiều", this, 20021, currItem));
                }
                else if (typeShop == 4)
                {

    ...
    public void paint(mGraphics g, int x, int y, int dir)
    {
        ...
        if (says != null && says.Length != 0 && type != 1)
        {
            ...
            for (int i = 0; i < says.Length; i++)
            {
                ...
                //    return;
                //}
                info.charInfo.paintHead(g, X + 8, Y + H / 2, 0);
                if (mGraphics.zoomLevel == 1)
                {
                    ((!info.isChatServer) ? mFont.tahoma_7b_green: mFont.tahoma_7b_yellow).drawString(g, info.charInfo.cName, X + 12, Y + 3, 0);
                }
                else
                {
                    ((!info.isChatServer) ? mFont.tahoma_7b_green : mFont.tahoma_7b_yellow).drawString(g, info.charInfo.cName, X + 12, Y - 3, 0);
                ...
                if (!GameCanvas.isTouch)
                {
                    if (!TField.isQwerty)
                    {
                        mFont.tahoma_7_grey.drawString(g, "Nhấn # để chat", X + W / 2 + 10, Y + H, mFont.CENTER);
                    }
                    else
                    {
                        mFont.tahoma_7_grey.drawString(g, "Nhấn Y để chat", X + W / 2 + 10, Y + H, mFont.CENTER);
                    }
                }
                if (mGraphics.zoomLevel == 1)
                {
                ...
                string[] array2 = mFont.tahoma_7_grey.splitFontArray(text, 120);
                for (int j = 0; j < array2.Length; j++)
                {
                    mFont.tahoma_7_grey.drawString(g, array2[j], X + 12, Y + 12 + j * 12 - 3, 0);
                }

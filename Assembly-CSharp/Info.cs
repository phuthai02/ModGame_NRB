public class Info : IActionListener
{
	...
	public void paint(mGraphics g, int x, int y, int dir)
	{
		...
		if (says != null && says.Length != 0 && type != 1)
		{
			...
			{
				mSystem.paintPopUp2(g, X + 21, Y - num / 2, W + 15, H + ((!GameCanvas.isTouch) ? 14 : 0) + num);
			}
			if (info.charInfo == null)
			{
			...
			for (int i = 0; i < says.Length; i++)
			{
				...
				info.charInfo.paintHead(g, X + (GameCanvas.w * 9 / 100) + 8 , Y + H / 2 - 2, 0);

                if (mGraphics.zoomLevel == 1)
				{
					((!info.isChatServer) ? mFont.tahoma_7b_greenSmall : mFont.tahoma_7b_yellowSmall).drawString(g, info.charInfo.cName, X + (GameCanvas.w * 9 / 100) + 10, Y + 5, 0);
				}
				else
				{
					((!info.isChatServer) ? mFont.tahoma_7b_greenSmall : mFont.tahoma_7b_yellowSmall).drawString(g, info.charInfo.cName, X + (GameCanvas.w * 9 / 100) + 10, Y - 1, 0);
				}
				...
				if (mGraphics.zoomLevel == 1)
				{
					TextInfo.paint(g, text, X + 10, Y + H / 2 + 2, W - 16, H, mFont.tahoma_7_whiteSmall);
					continue;
				}
				string[] array2 = mFont.tahoma_7_whiteSmall.splitFontArray(text, 120);
				for (int j = 0; j < array2.Length; j++)
				{
					mFont.tahoma_7_whiteSmall.drawString(g, array2[j], X + (GameCanvas.w * 9 / 100) + 10, Y + 12 + j * 12 - 3, 0);
				}
